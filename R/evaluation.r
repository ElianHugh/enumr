.check_eval <- function(index, obj) {
    ...
    # should be a more elegant and futureproof way of doing this
    mathOps <- c("+", "-", "*", "^", "%%", "%/%", "/", "==", ">", "<", "!=", "<=", ">=", "abs", "sign", "sqrt", "ceiling", "floor", "trunc", "cummax", "cummin", "cumprod", "cumsum", "exp", "expm1", "log", "log10", "log2", "log1p", "cos", "cosh", "sin", "sinh", "tan", "tanh", "acos", "acosh", "asin", "asinh", "atan", "atanh", "cospi", "sinpi", "tanpi", "gamma", "lgamma", "digamma", "trigamma")

    if (is.character(obj_val)) {
        return(FALSE)
        ...
        return(any(sym_name == ""))
    } else if (any(all.names(obj_val) %in% mathOps)) {
        return(TRUE)
    } else {
